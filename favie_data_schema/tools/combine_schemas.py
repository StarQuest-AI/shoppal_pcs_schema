    ...
    if isinstance(schema, dict):
        if 'type' in schema:
            if isinstance(schema['type'], list):
                # Handle union types
                schema['type'] = [resolve_references(t, combined_schemas) if isinstance(t, str) and t in combined_schemas else t for t in schema['type']]
...
def main():
    ...
    parser.add_argument('--source-directory', type=str, required=True, help='Path to the directory containing schema files.')
    parser.add_argument('--output-file', type=str, required=True, help='Path to the output file where the resolved schema will be written.')
    parser.add_argument('--main-schema', type=str, required=True, help='The full name of the main schema to resolve (including namespace).')
